...
const RoomCard = ({ room }) => {
  console.log('RoomCard - room.image:', room.images); // Debugging line to check room images
  return (
    <Card className="card-custom h-100">
      {/* <Card.Img variant="top" src={`http://localhost:8000/${room.images}`} /> */}
<Card.Img
        variant="top"
        src={
          room.images[0]?.startsWith('/uploads/')
          ? `http://localhost:8000${room.images[0]}`
          : `http://localhost:8000/uploads/${room.images[0]}`
        }
      />
      <Card.Body>
